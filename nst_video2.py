...
def model_nn(sess, input_image, train_step, count, frames_interval,
    ...
    clear()
    print("frames interval: " + str(frames_interval[0]) + " to " +
          str(frames_interval[1]) + "\n")
    print("running the model on frame " + str(count+1-frames_interval[0]) +
          "/" + str(frames_interval[1] - frames_interval[0]) + " ...\n")
...
if __name__ == "__main__":
    ...
    # frames_interval = np.array((int(sys.argv[3]), int(sys.argv[4])))[indices.astype(int)]
    frames_interval = [int(sys.argv[3]), int(sys.argv[4])]
    ...
    while(video.isOpened()):
        ...
        # run the model
        model_nn(sess, initialized_image, train_step, count, frames_interval,
                 300, output_folder)
        ...
        imwrite(output_folder + "/frame_" + str(count).zfill(3) + ".jpg",
                resized, dpi=(300,300))
        sess.close()
    video.release()
